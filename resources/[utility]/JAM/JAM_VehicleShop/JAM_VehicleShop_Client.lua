			...
			if nearestDist < self.DrawTextDist then
				...
				elseif listType == 2 or listType == 3 then
					for k,v in pairs(self.ShopData.Vehicles) do 
						if v.model == nearestVeh then 
							vehName = v.name
							vehPrice = tostring(v.price)
							extraStr = "Press [ Y ] to change vehicle."
							nearestModel = v.model
							nearestPrice = v.price
						end
					end
              ...
              if type(extraStr) == "number" then
                if plyJob == self.CarDealerJobLabel then
					self:DrawText3D(nearestPos.x,nearestPos.y,nearestPos.z + 1.0, "[ "..vehName.." ] : [ $" .. math.floor(vehPrice + (extraStr * vehPrice) / 100) .. " ]")     --self:DrawText3D(nearestPos.x,nearestPos.y,nearestPos.z + 1.0, "[ "..vehName.." ] : [ $" .. math.floor(vehPrice + (extraStr * vehPrice) / 100) .. " ] : [ "..extraStr.."% ]")
                else
				...
				elseif (IsControlJustPressed(0, JUtils.Keys["Y"], IsDisabledControlJustPressed(0, JUtils.Keys["Y"]))) then 
				...
				elseif (IsControlJustPressed(0, JUtils.Keys["F"], IsDisabledControlJustPressed(0, JUtils.Keys["F"]))) and plyJob == self.CarDealerJobLabel then 
					...
					if type(extraStr) == "number" then
						if plyJob == self.CarDealerJobLabel then
							...
							self:DrawText3D(nearestPos.x,nearestPos.y,nearestPos.z + 1.0, "[ "..vehName.." ] : [ $" .. math.floor(vehPrice + (extraStr * vehPrice) / 100) .. " ]")
							self:DrawText3D(nearestPos.x,nearestPos.y,nearestPos.z + 0.9, "[Press [ E ] to purchase this vehicle]")
...
function JVS:OpenUtilityMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Utility_Menu", { title = "PDM Sales", align = 'top', elements = elements }, 
		function(data,menu)
			menu.close()
...
function JVS:OpenSalesMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Sales_Menu", { title = "PDM Sales", align = 'top', elements = elements }, 
		function(data,menu)
			menu.close()
...
function JVS:OpenCategoryMenu(category)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Category_Menu", { title = "PDM : "..category.label, align = 'top', elements = elements }, 
		function(data,menu)
			menu.close()
...
function JVS:OpenDealerMenu()
	...
	if plyData.job.grade >= 3 then
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Dealer_Menu", { title = "PDM Dealer", align = 'top', elements = { [1] = {label = "Rearrange Displays"}, [2] = {label = "Dealership Account"} } }, 
			function(data,menu)
				menu.close()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Dealer_Menu", { title = "PDM Dealer", align = 'top', elements = { [1] = {label = "Rearrange Displays"} } }, 
			function(data,menu)
				menu.close()
...
function JVS:OpenDealerInventory()
	ESX.TriggerServerCallback('JAM_VehicleShop:GetDealerMoney', function(money)
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Dealer_Menu", { title = "PDM Inventory : $"..money, align = 'top', elements = { [1] = {label = "Deposit"}, [2] = {label = "Withdraw"} } }, 
			function(data,menu)
...
function JVS:OpenRearrangeMenu()
	...
	for k,v in pairs(self.DisplayVehicles) do
		local prefix = ''
		if k == 1 or k == 2 or k == 3 or k == 7 or k == 8 or k == 9 then prefix = "[Car] " else prefix = "[Bike] "; end 
		table.insert(elements, {label = prefix..self.ShopData.Displays[k].name, key = k})
	end

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Rearrange_Menu", { title = "Rearrange Vehicles", align = 'top', elements = elements }, 
		function(data,menu)
			menu.close()
...
function JVS:OpenSwapMenu(key)
	...
	for k,v in pairs(self.ShopData.Imports) do
		if key == 1 or key == 2 or key == 3 or key == 7 or key == 8 or key == 9 then
			if v.category == "importcars" then
				table.insert(elements, {label = v.name, model = v.model, price = v.price})
			end
		elseif key == 4 or key == 5 or key == 6 or key == 10 or key == 11 or key == 12 then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Import_Menu", { title = "Import Vehicles", align = 'top', elements = elements }, 
		function(data,menu)		
