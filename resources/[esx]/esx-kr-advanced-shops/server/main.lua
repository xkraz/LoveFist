...
AddEventHandler('esx_kr_shops:RemoveItemFromShop', function(number, count, item)
  ...
  local identifier =  ESX.GetPlayerFromId(src).identifier
        ...
        'SELECT count, item FROM player_shops WHERE item = @item AND ShopNumber = @ShopNumber',
        {
            ['@ShopNumber'] = number,
            ['@item'] = item,
        },
        function(data)
            ...
            if count > data[1].count then
                ...
                if data[1].count ~= count then
                    ...
                    MySQL.Async.fetchAll("UPDATE player_shops SET count = @count WHERE item = @item AND ShopNumber = @ShopNumber",
                    {
                        ['@item'] = item,
                        ['@ShopNumber'] = number,
                ...
                elseif data[1].count == count then
                    ...
                    MySQL.Async.fetchAll("DELETE FROM player_shops WHERE item = @name AND ShopNumber = @Number",
                    {
                        ['@Number'] = number,
                        ['@name'] = data[1].item
                    })
...
AddEventHandler('esx_kr_shops:setToSell', function(id, Item, ItemCount, Price)
  ...
  MySQL.Async.fetchAll(
    ...
    function(items)
      ...
      MySQL.Async.fetchAll(
        'SELECT price, count FROM player_shops WHERE item = @items AND ShopNumber = @ShopNumber',
        {
            ['@items'] = Item,
            ['@ShopNumber'] = id,
        },
        ...
        if data[1] == nil then
            ...
            MySQL.Async.execute('INSERT INTO player_shops (ShopNumber, src, label, count, item, price) VALUES (@ShopNumber, @src, @label, @count, @item, @price)',
            {
                ['@ShopNumber']    = id,
                ['@src']        = imgsrc,
                ['@label']         = items[1].label,
            ...
            elseif data[1].price == Price then
                ...
                MySQL.Async.fetchAll("UPDATE player_shops SET count = @count WHERE item = @name AND ShopNumber = @ShopNumber",
                {
                    ['@name'] = Item,
                    ['@ShopNumber'] = id,
...
AddEventHandler('esx_kr_shops:Buy', function(id, Item, ItemCount)
  ...
  local xPlayer = ESX.GetPlayerFromId(src)
        ...
        MySQL.Async.fetchAll(
        'SELECT * FROM player_shops WHERE ShopNumber = @Number AND item = @item',
        {
            ['@Number'] = id,
            ['@item'] = Item,
        }, function(result)
        ...
        'SELECT * FROM owned_shops WHERE ShopNumber = @Number',
        {
            ...
            elseif ItemCount <= 0 then
                ...
                if result[1].count ~= ItemCount then
                    MySQL.Async.execute("UPDATE player_shops SET count = @count WHERE item = @name AND ShopNumber = @Number",
                    {
                        ['@name'] = Item,
                        ['@Number'] = id,
...
ESX.RegisterServerCallback('esx_kr_shop:getShopItems', function(source, cb, number)
  local identifier = ESX.GetPlayerFromId(source).identifier
        ...
        MySQL.Async.fetchAll('SELECT * FROM player_shops WHERE ShopNumber = @ShopNumber',
        {
            ['@ShopNumber'] = number
        }, function(result)
        cb(result)
...
AddEventHandler('esx_kr_shops:SellShop', function(number)
  ...
  MySQL.Async.fetchAll(
    ...
    function(result)
      MySQL.Async.fetchAll(
        'SELECT * FROM player_shops WHERE ShopNumber = @ShopNumber',
        {
          ['@ShopNumber'] = number,
        },
        function(result2)
