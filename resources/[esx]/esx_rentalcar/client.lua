...
Citizen.CreateThread(function()
    while true do
		...
		if(Config.TypeMarker ~= -1 and GetDistanceBetweenCoords(coords, Config.MarkerZones[k].x, Config.MarkerZones[k].y, Config.MarkerZones[k].z, true) < Config.DrawDistance) then
            DrawMarker(Config.TypeMarker, Config.MarkerZones[k].x, Config.MarkerZones[k].y, Config.MarkerZones[k].z, 0, 0, 0, 0, 0, -65.0, Config.MarkerScale.x, Config.MarkerScale.y, Config.MarkerScale.z, Config.MarkerColor.r, Config.MarkerColor.g, Config.MarkerColor.b, 100, 1, 0, 0, 0)	
		end
    end
	end
end)
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
				...
				elseif havebike == true then
					...
					if IsControlJustPressed(0, Config.Key) and IsPedInAnyVehicle(ped) then
...
function OpenBikesMenu()
	...
	if Config.EnablePrice == false then
		...
		table.insert(elements, {label = _U('bike6free'), value = 'bike6'}) 
	...
	if data.current.value == 'bike' then
		...
		if Config.EnableEffects then
			...
			TriggerEvent('esx:spawnVehicle', "seb5")
		end
	end
	...
	if data.current.value == 'bike2' then
		...
		if Config.EnableEffects then
			...
			TriggerEvent('esx:spawnVehicle', "hrtf1")
		end
	...
	if data.current.value == 'bike3' then
		...
		if Config.EnableEffects then
			spawn_effect("lotus11")
		else
			TriggerEvent('esx:spawnVehicle', "lotus11")
		end
	end
	...
	if data.current.value == 'bike4' then
		...
		if Config.EnableEffects then
			spawn_effect("mercedesf1")
		else
			TriggerEvent('esx:spawnVehicle', "mercedesf1")
		end
	end
		...
		if Config.EnableEffects then
			spawn_effect("redbull")
		else
			TriggerEvent('esx:spawnVehicle', "redbull")
		end
	end
	
		if data.current.value == 'bike6' then
		if Config.EnablePrice then
			TriggerServerEvent("esx:bike:lowmoney", Config.PriceBmx) 
			TriggerEvent("chatMessage", _U('bikes'), {255,0,255}, _U('bike_pay', Config.PriceBmx))
			ESX.ShowNotification(_U('enjoy_bike'))
		...
		if Config.EnableEffects then
			spawn_effect("willif1")
		else
			TriggerEvent('esx:spawnVehicle', "willif1")
		end
	end
