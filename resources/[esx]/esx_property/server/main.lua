...
function RemoveOwnedProperty(name, price, owner)
	MySQL.Async.execute('DELETE FROM owned_properties WHERE name = @name AND owner = @owner',
	{
		...
		if xPlayer then
			...
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('made_property', ESX.Math.GroupDigits(price)))     --JINKS DOGGERY
		end
	end)
end
...
AddEventHandler('esx_property:removeOwnedProperty', function(name, price, owner)            --Jinks DOGGERY undone
	RemoveOwnedProperty(name, price, owner)
end)
...
AddEventHandler('esx_property:buyProperty', function(propertyName)
	...
	if property.price <= xPlayer.getBank() then
		xPlayer.removeAccountMoney('bank', property.price)
		xPlayer.removeBank(property.price)
...
AddEventHandler('esx_property:removeOwnedProperty', function(propertyName)
	local xPlayer = ESX.GetPlayerFromId(source)
	local property = GetProperty(propertyName)
	xPlayer.addAccountMoney('bank', property.price)
	RemoveOwnedProperty(propertyName, property.price, xPlayer.identifier)
end)
