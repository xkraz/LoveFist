...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		--TriggerServerEvent('esx_marducasjob:onNPCJobCompleted')
	end
end

function OpenmarducasActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'marducas_actions', {
		title    = _U('marducas'),
		align    = 'top',
		elements = elements
	}, function(data, menu)
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						TriggerServerEvent('esx_society:removeVehicleFromGarage', 'marducas', vehicleProps)
...
---
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'marducas', function(data, menu)
				menu.close()
			end)
		end
		...
		CurrentAction     = 'marducas_actions_menu'
		CurrentActionMsg  = _U('open_actions')
		CurrentActionData = {}
	end)
end

function OpenmarducasHarvestMenu()
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'marducas_harvest', {
			title    = _U('harvest'),
			align    = 'top',
			elements = elements
		}, function(data, menu)
			...
			if data.current.value == 'gaz_bottle' then
				TriggerServerEvent('esx_marducasjob:startHarvest')
			elseif data.current.value == 'fix_tool' then
				TriggerServerEvent('esx_marducasjob:startHarvest2')
			elseif data.current.value == 'caro_tool' then
				TriggerServerEvent('esx_marducasjob:startHarvest3')
			end
		end, function(data, menu)
			menu.close()
			CurrentAction     = 'marducas_harvest_menu'
...
function OpenmarducasCraftMenu()
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'marducas_craft', {
			title    = _U('craft'),
			align    = 'top',
			elements = elements
		}, function(data, menu)
			...
			if data.current.value == 'blow_pipe' then
				TriggerServerEvent('esx_marducasjob:startCraft')
			elseif data.current.value == 'fix_kit' then
				TriggerServerEvent('esx_marducasjob:startCraft2')
			elseif data.current.value == 'caro_kit' then
				TriggerServerEvent('esx_marducasjob:startCraft3')
			end
		end, function(data, menu)
			menu.close()

			CurrentAction     = 'marducas_craft_menu'
...
function OpenMobilemarducasActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_marducas_actions', {
		title    = _U('marducas'),
		align    = 'top',
		elements = {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 or amount > 10000 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_marducas', _U('marducas'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
				...
				if NPCOnJob then
					if NPCTargetDeleterZone then
						...
						if CurrentlyTowedVehicle == NPCTargetTowable then
							...
							TriggerServerEvent('esx_marducasjob:onNPCJobMissionCompleted')
							StopNPCJob()
	...
	elseif data.current.value == 'object_spawner' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_marducas_actions_spawn', {
			title    = _U('objects'),
			align    = 'top',
			elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_marducasjob:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_marducasjob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_marducasjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_marducasjob:putStockItems', itemName, count)
...
RegisterNetEvent('esx_marducasjob:onCarokit')
AddEventHandler('esx_marducasjob:onCarokit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	local nums = {
		eng = -1,
...
function checkDistance(car)

	local pl = GetEntityCoords(GetPlayerPed(PlayerId()), true)
	local p2 = hoodLocation(car)

	print(GetDistanceBetweenCoords(pl, p2, true))
	if GetDistanceBetweenCoords(pl, p2, true) <= 1.6 then
		return true
	else
		return false
	end
...
function repairLocation(car)
	local dim = GetModelDimensions(GetEntityModel(car), vector3(0.0, 0.0, 0.0), vector3(10.0, 10.0, 10.0))
	return GetOffsetFromEntityInWorldCoords(car, 0.0, (dim.y*-1), 0.0)
end

function hoodLocation(car)
	local dim = GetModelDimensions(GetEntityModel(car), vector3(0.0, 0.0, 0.0), vector3(10.0, 10.0, 10.0))
	return GetOffsetFromEntityInWorldCoords(car, 0.0, (dim.y*-1) - 1, 0.0)
end
...
AddEventHandler('esx_marducasjob:hasEnteredMarker', function(zone)
	if zone == NPCJobTargetTowable then

	elseif zone =='VehicleDelivery' then
		NPCTargetDeleterZone = true
	elseif zone == 'marducasActions' then
		CurrentAction     = 'marducas_actions_menu'
		CurrentActionMsg  = _U('open_actions')
		CurrentActionData = {}
...
AddEventHandler('esx_marducasjob:hasExitedMarker', function(zone)
	if zone =='VehicleDelivery' then
		NPCTargetDeleterZone = false
	elseif zone == 'Craft' then
		TriggerServerEvent('esx_marducasjob:stopCraft')
		TriggerServerEvent('esx_marducasjob:stopCraft2')
		TriggerServerEvent('esx_marducasjob:stopCraft3')
	elseif zone == 'Garage' then
		TriggerServerEvent('esx_marducasjob:stopHarvest')
		TriggerServerEvent('esx_marducasjob:stopHarvest2')
		TriggerServerEvent('esx_marducasjob:stopHarvest3')
	end
...
AddEventHandler('esx_marducasjob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'marducas' and not IsPedInAnyVehicle(playerPed, false) then
...
AddEventHandler('esx_marducasjob:hasExitedEntityZone', function(entity)
...
Citizen.CreateThread(function()
	...
	AddTextComponentSubstringPlayerName(_U('marducas'))
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'marducas' then
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'marducas' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_marducasjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_marducasjob:hasExitedMarker', LastZone)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_marducasjob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('esx_marducasjob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, Keys['E']) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'marducas' then

				if CurrentAction == 'marducas_actions_menu' then
					OpenmarducasActionsMenu()
				elseif CurrentAction == 'marducas_harvest_menu' then
					OpenmarducasHarvestMenu()
				elseif CurrentAction == 'marducas_craft_menu' then
					OpenmarducasCraftMenu()
				elseif CurrentAction == 'delete_vehicle' then

					if Config.EnableSocietyOwnedVehicles then

						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData.vehicle)
						TriggerServerEvent('esx_society:putVehicleInGarage', 'marducas', vehicleProps)
						...
						if
							...
							GetEntityModel(vehicle) == GetHashKey('raptor12')
						then
							TriggerServerEvent('esx_service:disableService', 'marducas')
						end
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'marducas' then
			OpenMobilemarducasActionsMenu()
		end

		if IsControlJustReleased(0, Keys['DELETE']) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'marducas' then
			if NPCOnJob then
