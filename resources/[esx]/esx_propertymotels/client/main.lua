...
function OpenPropertyMenu(property)
	...
	if PropertyIsOwned(property) then
		...
		if not Config.EnablePlayerManagement then
			...
			table.insert(elements, {label = _U('rent_motel')..' for $'..Config.MotelRentalPrice, value = 'rent'})
			else
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'property',
	{
		...
		elseif data.current.value == 'leave' then
			...
			TriggerEvent("mythic_progbar:client:progress", {
				name = "renting_motel",
				duration = Config.PedActionTime,
				label = "Cancelling Rental Contract",
				useWhileDead = false,
				canCancel = true,
				controlDisables = {
						disableMovement = true,
...
Citizen.CreateThread(function()
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("The Pink Cage Motel")
