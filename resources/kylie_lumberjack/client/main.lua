	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'citizen_wear' then
				...
				TriggerServerEvent("player:serviceOff", "lumberjack")
			...
			if data.current.value == 'lumberjack_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
				TriggerServerEvent("player:serviceOn", "lumberjack")
					if skin.sex == 0 then
			...
			CurrentAction     = 'lumberjack_action_menu'
...
function OpenLumberjackActionsMenu()
	...
	if Config.EnablePlayerManagement and PlayerData.job ~= nil and  PlayerData.job.grade_name ~= 'recruit' then 
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = 'Work Truck',  value = 'sandking'},
			{label = 'Work ATV',  value = 'blazer'},
		}
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('kylie_lumberjack:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('kylie_lumberjack:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('kylie_lumberjack:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('kylie_lumberjack:putStockItems', itemName, count)
						end
...
AddEventHandler('kylie_lumberjack:hasEnteredMarker', function(zone)
	if zone == 'woodChopping' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'wood_chopping'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'woodPackaging' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'wood_packaging'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'plankCutting' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'plank_cutting'
...
AddEventHandler('kylie_lumberjack:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'woodChopping') and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		TriggerServerEvent('kylie_lumberjack:stopHarvest')
	end  
	if (zone == 'woodPackaging' or zone == 'plankCutting') and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		TriggerServerEvent('kylie_lumberjack:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		TriggerServerEvent('kylie_lumberjack:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		...
		AddTextComponentString("Lumberjacks")
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('kylie_lumberjack:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('kylie_lumberjack:hasExitedMarker', LastZone)
			end
		end
	end
end)
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'wood_chopping' then
					TriggerServerEvent('kylie_lumberjack:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'plank_cutting' then
					TriggerServerEvent('kylie_lumberjack:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'wood_packaging' then
					TriggerServerEvent('kylie_lumberjack:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('kylie_lumberjack:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'lumberjack_action_menu' then
