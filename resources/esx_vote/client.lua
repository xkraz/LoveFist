...
function OpenVoteMenu()
	...
	ESX.UI.Menu.Open(
		...
		{
			title    = "Re-elect Edwards for Mayor?",
			elements = {
				{label = "Re-elect Edwards for Mayor", value = 'Edwards'},
				{label = "Force a New Election", value = 'Election'},
				{label = "Blank Vote",  value = 'Blank Vote'},-- value = nil}
				{label = "Remove vote",  value = nil}
			}
		},
		function(data, menu)
			TriggerServerEvent("elec:vote",data.current.value)
			TriggerEvent('esx:showNotification', "You voted for "..data.current.label)
			menu.close()
		end,
		function(data, menu)
			menu.close()
