...
AddEventHandler('esx-qalle-hunting:sell', function()
    ...
    local MeatQuantity = xPlayer.getInventoryItem('casino_chip').count

    if MeatQuantity > 0 or LeatherQuantity > 0 then
        xPlayer.addMoney(MeatQuantity * MeatPrice)

        xPlayer.removeInventoryItem('casino_chip', MeatQuantity)
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'You sold ' .. MeatQuantity .. ' for $' .. MeatPrice * MeatQuantity)
    else
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'You don\'t have any Casino Chips')
    end
