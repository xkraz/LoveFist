...
ESX                           = nil
animBind = NativeUI.CreateMenu("LoveFist", "Availables actions", 5, 100)
...
function OpenBindMenu()
    ...
    bind = NativeUI.CreateListItem("Hotkey : ", pavInd, 0, "Select an animation first then select the hotkey")
    animq = NativeUI.CreateListItem("Animations : ", pavAnim, 0, "")
    pref:AddItem(bind)
    pref:AddItem(animq)


    p = NativeUI.CreateItem("Stop animations","")
    menu:AddItem(p)
    ...
    pref.OnListSelect = function(_, item, index)
        ...
        if item == bind then
            if selectedBindAnim ~= nil then
                TriggerEvent("dqp:shownotif", "Animation Bind : " .. selectedBindAnim .. "~n~on the hotkey " .. pavInd[index], 26)
                TriggerServerEvent("dqp:saveBind", selectedAnim, pavName[index])
            else
                TriggerEvent("dqp:shownotif", "No animation selected", 6)
            end
        end
        if item == animq then
            TriggerEvent("dqp:shownotif", "Selected animation : " .. pavAnim[index], 26)
...
AddEventHandler('dqp:ready', function()
    ESX.TriggerServerCallback("dqp:getsettings", function(result)
        ...
        if result[1] ~= nil then
            ...
            TriggerEvent("dqp:shownotif", "Settings loaded", 26)
        end
    end)
end)
