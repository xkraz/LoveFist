...
local drugs = "16 of each bag of drugs"
	...
	while true do
		Citizen.Wait(5000)
				if event_is_running then
						...
						if GetVehicleEngineHealth(event_vehicle) < 101 and event_vehicle ~= nil then
							ResetCargo()
...
function CargoMenu()
    ...
    for i = 1, #Config.Scenarios do
    	Menu.addButton("Cargo - " .. drugs, "PurchaseCargo", i)
   	end
...
function AlertThePolice()
	...
	if event_is_running then
    	...
    	TriggerServerEvent('esx_addons_gcphone:startCall', 'police', ":anon: Cargo Delivery in progress [ " .. vehicle_plate .. " ] !", PlayerCoords, {
    	...
    	for i = 1, #Config.AlertExtraSocieties do
    		...
    		if PlayerData.job.name ~= Config.AlertExtraSocieties[i] then
    			...
    			TriggerServerEvent('esx_addons_gcphone:startCall', Config.AlertExtraSocieties[i], ":anon: Cargo Delivery in progress [ " .. vehicle_plate .. " ]!", PlayerCoords, {
				PlayerCoords = { x = PedPosition.x, y = PedPosition.y, z = PedPosition.z },
				})
...
function DeliverCargo()
	...
	police_alerted = false
	local talktodealer = true
	TriggerServerEvent('esx_cargodelivery:cooldown')
...
function PurchaseCargo(scenario)
	...
	if event_is_running == true or completed_delivery == true then
	...
	ESX.TriggerServerCallback('esx_cargodelivery:getCopsOnline', function(police)
		...
		if police >= Config.Scenarios[scenario].MinCopsOnline then
			...
			if bought then
				...
				Citizen.Wait(12500)
				ExecuteCommand('cargo1', function(source, args)
				end)
				Citizen.Wait(12500)
				ExecuteCommand('cargo', function(source, args)
				end)
				Citizen.Wait(12500)
				ExecuteCommand('cargo', function(source, args)
				end)
...
RegisterNetEvent('cargo:setBlip')
AddEventHandler('cargo:setBlip', function(name, x, y, z)
...
RegisterNetEvent('cargo:killBlip')
AddEventHandler('cargo:killBlip', function()
    RemoveBlip(blip)
end)

-- Command -- 

RegisterCommand('cargo', function(source, args)
    ...
    if args[1] == nil then
        TriggerServerEvent('cargo', location, x, y, z, name)
    else
        TriggerServerEvent('cargo', location, x, y, z, name)
    end
end)

RegisterCommand('cargo1', function(source, args)
    local name = GetPlayerName(PlayerId())
    local ped = GetPlayerPed(PlayerId())
    local x, y, z = table.unpack(GetEntityCoords(ped, true))
    local street = GetStreetNameAtCoord(x, y, z)
    ...
    local msg = table.concat(args, ' ')
    if args[1] == nil then
        TriggerServerEvent('cargo1', location, x, y, z, name)
    else
        TriggerServerEvent('cargo1', location, x, y, z, name)
    end
end)
