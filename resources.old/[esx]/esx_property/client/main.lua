...
function CreateBlips()
	for i=1, #Config.Properties, 1 do
		...
		if property.entering then
			...
			SetBlipScale  (Blips[property.name], 0.55)
...
function SetPropertyOwned(name, owned)
	...
	if owned then
		...
		SetBlipScale  (Blips[enteringName], 0.55)
...
function OpenPropertyMenu(property)     
	local price = property.price	-- JINKS PUT THE FUCKING PRICE IN THE MENU
	...
	if PropertyIsOwned(property) then
		...
		--	table.insert(elements, {label = _U('leave_test', ESX.Math.GroupDigits(price)), value = 'leave'}) -- Jinks put the price in the daggum menu
		end
	else
		if not Config.EnablePlayerManagement then
			table.insert(elements, {label = _U('buy'), value = 'buy'})   --JINKS PUT THE FUCKING PRICE IN THE MENU
			--table.insert(elements, {label = _U('buy_test', ESX.Math.GroupDigits(price)), value = 'buy'})
