...
AddEventHandler('esx:playerconnected', function()
 ...
 for k,v in ipairs(GetPlayerIdentifiers(source))do
  ...
  local connectstring = GetPlayerName(source) .. " (" .. identifier .. ") (" .. license .. ")"
    sendToDiscord(_U('server_connecting'), connectstring .." ".. _('user_connecting'),Config.grey)
  end
end)
...
AddEventHandler('playerDropped', function(reason)
  ...
  local disconnectstring = GetPlayerName(source) .. " (" .. identifier .. ") (" .. license .. ")"
    ...
    sendToDiscord(_U('server_disconnecting'), disconnectstring .." ".. _('user_disconnecting') .. "("..reason..")",Config.grey)
  end
end)
...
AddEventHandler("esx:banhammer", function(name,staff)
  ...
  local banstring = GetPlayerName(source) .. " " .. identifier 
   
   
   
   
    sendToDiscordBan(_U('server_banhammer'), banstring .." ".._('banned_by').." "..name,Config.purple)
   end
...
-- Add event when a player deposit dirty money
-- TriggerEvent("esx:depositdirtymoneyalert",xPlayer.name,pickup.count) -> ESX_extended
RegisterServerEvent("esx:depositdirtymoneyalert")
AddEventHandler("esx:depositdirtymoneyalert", function(name,amount)
   if(settings.LogDirtyMoneyPickup)then
    sendToDiscord(_U('server_dirtymoney_deposit'),name.." ".._('user_deposit').." : "..amount .." " .._U('dirty_money'),Config.orange)
   end

end)

-- Add event when a player deposit dirty money 
-- TriggerEvent("esx:withdrawdirtymoneyalert",xPlayer.name,pickup.count) -> ESX_extended
RegisterServerEvent("esx:withdrawdirtymoneyalert")
AddEventHandler("esx:withdrawdirtymoneyalert", function(name,amount)
   if(settings.LogDirtyMoneyPickup)then
    sendToDiscord(_U('server_dirtymoney_withdraw'),name.." ".._('user_withdraw').." : "..amount .." " .._U('dirty_money'),Config.orange)
   end
...
AddEventHandler('esx:killerlog', function(t,killer, kilerT) -- t : 0 = NPC, 1 = player
  local xPlayer = ESX.GetPlayerFromId(source)
   ...
   for k,v in ipairs(GetPlayerIdentifiers(killer))do
							if string.sub(v, 1, string.len("steam:")) == "steam:" then
								identifier = v
							elseif string.sub(v, 1, string.len("license:")) == "license:" then
								license = v
  ...
  local killerid = GetPlayerName(killer) .. " " .. identifier  
     ...
     if(xPlayerKiller.name ~= nil and xPlayer.name ~= nil)then
       ...
       if(kilerT.killerinveh) then
         local model = kilerT.killervehname
            ...
            sendToDiscord(_U('server_kill_vdm'), xPlayer.name .." ".._('user_vdm').." "..killerid.." ".._('with').." "..model,Config.red)



       else
            sendToDiscord(_U('server_kill'), xPlayer.name .." ".._('user_kill').." "..killerid,Config.red)
