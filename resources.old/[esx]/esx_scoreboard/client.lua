...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['PAGEDOWN']) and IsInputDisabled(0) then
			ToggleScoreBoard()
			Citizen.Wait(200)
		elseif IsControlJustReleased(0, Keys['PAGEDOWN']) and IsInputDisabled(0) then
			SendNUIMessage({
				action  = 'close'
			})
		end
	    if IsControlPressed(0, 37) and IsControlPressed(0, 45) and not IsInputDisabled(0) then
			ToggleScoreBoard()
			Citizen.Wait(200)
		elseif IsControlPressed(0, 37) and IsControlPressed(0, 45) and not IsInputDisabled(0) then
			SendNUIMessage({
				action  = 'close'
			})
		end
